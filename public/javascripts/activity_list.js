...
var tdMap = {
    ...
    'deletelink':function(act, key) {
        ...
        var now = new Date();
        if(now >= getDateByObj(act.book_start) && now < getDateByObj(act.book_end) && act.status == 1){
            duringbook.push(act[key]);
            return '<span id="del'+act[key]+'" class="td-ban glyphicon glyphicon-ban-circle" ></span>';
        }
        else if(now >= getDateByObj(act.book_end) && now < getDateByObj(act.start_time) && act.status == 1){
            beforeact.push(act[key]);
            return '<span id="del'+act[key]+'" class="td-ban glyphicon glyphicon-ban-circle" ></span>';
        }
        else if(now >= getDateByObj(act.start_time) && now < getDateByObj(act.end_time) && act.status == 1){
